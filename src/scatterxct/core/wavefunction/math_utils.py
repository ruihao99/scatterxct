...
def calculate_other_populations(psi: ArrayLike, U: ArrayLike, dR: float, mode: str) -> ArrayLike:
    """The population for each state.
    ...
    _calculate_other_populations(psi, psi_other_rep, U, mode=mode)
    return np.sum(get_nuclear_density(psi_other_rep, dR), axis=0)

@njit
def _calculate_other_populations(psi: ArrayLike, psi_other_rep: ArrayLike, U: ArrayLike, mode: str) -> ArrayLike:
    nstates, _, ngrid = U.shape
    psi_tmp = np.zeros((nstates, ), dtype=np.complex128)
    transform = np.zeros((nstates, nstates), dtype=np.complex128)
    for ii in range(ngrid):
        if mode == "diabatic":
            transform[:] = U[:, :, ii].conj().T
        elif mode == "adiabatic":
            transform[:] = U[:, :, ii]
            
        psi_tmp[:] = np.dot(transform, psi[ii, :])
...
@njit
def get_diag_H(E: ArrayLike,):
    nstates, ngrid = E.shape
    H = np.zeros((nstates, nstates, ngrid), dtype=np.complex128)
    for ii in range(ngrid):
        H[:, :, ii] = np.diagflat(E[:, ii])
    return H
